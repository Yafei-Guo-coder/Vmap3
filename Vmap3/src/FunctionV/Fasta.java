...
//        }
    ...
    public static ArrayList<String> randomSequences(ArrayList<ArrayList<String>> filterBlock, ArrayList<Integer> startPos,int num1, int num2){
        ArrayList<String> res = new ArrayList<>();
//        ArrayList<Integer> startPos = new ArrayList<>();
//        for (ArrayList<String> gene: filterBlock) {
//            int p = getRandom(0, gene.get(1).length()-351);
...
//        }
        for (int i = 0; i < filterBlock.size(); i++) {
            for (int j = 0; j < num1; j++) {
                res.add((String) filterBlock.get(0).get(1).subSequence(startPos.get(j), startPos.get(j)+350));
            }
            for (int j = num1; j < num2; j++) {
                res.add((String) filterBlock.get(1).get(1).subSequence(startPos.get(j), startPos.get(j)+350));
    ...
    public static ArrayList<Integer> getRandomStartPos(ArrayList<ArrayList<String>> filterBlock, int num1,int num2){
        ArrayList<Integer> startPos = new ArrayList<>();
        for (ArrayList<String> gene: filterBlock) {
            if(filterBlock.indexOf(gene)==0) {
